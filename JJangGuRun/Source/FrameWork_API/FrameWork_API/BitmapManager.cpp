...
void CBitmapManager::LoadMenuImg()
{
	...
	CString tDir = L"Resources/Image/Menu/MainMenu.bmp";
...
void CBitmapManager::LoadBackImg()
{
	...
	for (int i = 0; i < NUM_OF_BLOCK; ++i) {
		...
		CString tDir;
		CString tKey;
		tKey.Format(L"BLOCK%d", i + 1);
		tDir.Format(L"Resources/Image/Block/Block%d.bmp", i + 1);
		pImg->Load(tDir);
		m_map.insert(make_pair(tKey, pImg));
	}
	...
	for (int i = 0; i < NUM_OF_BULLET; ++i) {
		...
		CString tDir;
		CString tKey;
		tKey.Format(L"BULLET%d", i + 1);
		tDir.Format(L"Resources/Image/Bullet/Bullet%d.bmp", i + 1);
		pImg->Load(tDir);
		m_map.insert(make_pair(tKey, pImg));
	}
	...
	for (int i = 0; i < NUM_OF_ENEMY; ++i) {
		...
		tDir.Format(L"Resources/Image/Enemy/Enemy%d.png", i + 1);
		pImg->Load(tDir);
		m_map.insert(make_pair(tKey, pImg));
	}

	{
		...
		CString tKey;
		tKey = L"BACK";
		tDir = L"Resources/Image/Back/Back.bmp";
		pImg->Load(tDir);
		m_map.insert(make_pair(tKey, pImg));
...

	{
		...
		CString tKey;
		tKey = L"GAMECLEAR";
		tDir = L"Resources/Image/GameClear/GameClear.png";
		pImg->Load(tDir);
		m_map.insert(make_pair(tKey, pImg));
