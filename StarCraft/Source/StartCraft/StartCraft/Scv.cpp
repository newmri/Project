...
void CScv::LateInit()
{
	...
	p = BITMAPMANAGER->GetImageInfo(CONTROL_BUILD);
	for (int i = 0; i < p[0].nImageNum; ++i) {
		STATIC_UI_IMAGE_INFO* pInfo = new STATIC_UI_IMAGE_INFO;

		memcpy(&pInfo->tInfo, &p[i], sizeof(IMAGE_INFO));
		m_buildImageList[CONTROL].push_back(pInfo);
...
bool CScv::CheckCommand(POINT tMousePos)
{
	int nCnt = 0;
	if (BASIC == m_eCommand) {
...
void CScv::Attack()
{
	if (SCV_STATE::MINE == m_eCurrId && m_dwAttackTime + 50 < GetTickCount()) {
...
void CScv::BuildStructure(OBJ_ID eId, INTPOINT pos, float fAngle)
{
	m_eBuildId = eId;
	m_BuildPos = pos;
